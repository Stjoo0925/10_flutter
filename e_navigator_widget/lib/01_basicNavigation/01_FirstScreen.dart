...
import '../02_tabNavigation/01_TabScreen.dart';
  ...
  Widget build(BuildContext context) {
    return ScaffoldWidget(widget: Center(
      child: Column(
        ...
        children: [
          const Text("첫 번쨰 페이지 입니다."),
          ElevatedButton(onPressed: (){
            Navigator.pushNamed(context, "/second");
          }, child: const Text("페이지 이동")),
          ElevatedButton(onPressed: (){
            Navigator.pushNamed(context, "/bottom");
          }, child: const Text("bottom 이동"))
        ],
      ),
    ) );
  }
...
///
/// 주요 속성
/// builder :  새로운 화면을 생성하는 함수이다. 이 함수는 builderContext를 매개변수로 받아 새로운 위젯을 반환한다.
/// setting : 라우트의 설정 정보를 담고 있는 객체로 라우트의 이름이나 인자를 전달할 때 사용할 수 있다.
