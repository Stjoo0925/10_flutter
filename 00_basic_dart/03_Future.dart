 ...
 * 
 * 비동기함수 : async 키워드로 정의된 함수에서 Future를 반환한다.
 * 결과 처리 : it를 사용하여 Future의 완료를 기다리거나, then 메서드를 사용하여 결과를 처리한다.
 * 오류 처리 : catchError 메서드를 통해 비동기 작업에서 발생한 오류를 처리할 수 있다.
 * Future는 다트의 비동기 프로그래밍에서 중요한 역활을 하며, 사용자 경험을 향상시키는 데 큰 도움이 된다.
...
Future<String> fecthData() async {
  // throw Exception("확인");
  await Future.delayed(Duration(seconds: 2)); // url 요청
  return "데이터 로드 완료";
}

void main() async {
  // String result = await fecthData();
  // print(result);
