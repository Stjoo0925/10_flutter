...
class MediaQueryWidget extends StatelessWidget {
  ...
  /// Column
  /// Column은 flutter에서 수직 방향으로 여러 위젯을 배치할 떄 사용하는 레이아웃 위젯이다.
  ...
  /// crossAxisAlignment : 수평 방향(교차 축)에서 자식 위젯의 정렬 방식을 설정한다.
  /// mainAxisSize : 주 축의 크기를 설정한다. MainAxisSize.max(기본값) 또는 MainAxisSize.main을 사용할 수 있다.
  /// verticalDirection : 자식 위젯의 배치 방향을 설정한다. VerticalDirection.down(기본값) 또는 VerticalDirection.up을 선택할 수 있다.

  @override
  Widget build(BuildContext context) {
    /// MediaQuery
    /// 플러터에서 디바이스의 화면크기, 방향, 밀도 등의 정보를 제공하는 위젯으로
    ...
    return Column(
      children: [
        ...
        Text("화면의 높이 : ${mediaQuery.size.height * 1}"),
        // Flutter 에서 화면의 방향을 세로 모드롤 설정하는 데 사용하는 함수
        Text("화면의 방향 : ${mediaQuery.orientation == Orientation.portrait
            ? "세로"
            : "가로"}"),
